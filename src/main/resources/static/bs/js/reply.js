...
function getTest() {
	$("#Comment_regist").click(function(){
		var msg = {
			...
			inus_CmContent : $("input#inus_CmContent").val() // 작성하는 내용을 벨류값으로 가져온다.
		}
		console.log($("input#inus_CmContent").val());
		commonajax("/comment/InsertComment", msg, "get", function(result){
			getList(result);
		});
			
		
		$("input#inus_CmContent").val("");
	});
}
	...
	function getList(data) {
		...
		if(data.list.length > 0) {
			...
			for(i = 0; i < list.length; i++){
				...
				comment_html += "<span id='inus_CmWriter'><strong>" + writer + "</strong></span>&nbsp;&nbsp;";
